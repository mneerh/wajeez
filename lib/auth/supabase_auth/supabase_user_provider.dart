...
class WajeezAppSupabaseUser extends BaseAuthUser {
  WajeezAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> wajeezAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = WajeezAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
